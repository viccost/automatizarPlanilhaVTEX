...
def salvarArquivo(planilha: pd.DataFrame, nome: str, formatar=True):
    ...
    checarCaminho()
    writer = pd.ExcelWriter(rf'{caminhoDesktop}{nome}.xlsx', engine='xlsxwriter')
    # , engine_kwargs={'options': {'strings_to_numbers': True}}
    planilha.to_excel(writer, sheet_name='VTEX 1 Limpa', index=False)
    ...
    def ajustarColunas():
        for column in planilha:
            ...
            writer.sheets['VTEX 1 Limpa'].set_column(col_idx, col_idx, column_width)

    def setarTipo():
        workbook = writer.book
        worksheet = writer.sheets['VTEX 1 Limpa']
